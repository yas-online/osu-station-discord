...
function FormatLog( hOptions )
{
	...
	if( hOptions.showLevel )
	{
		...
		sLog += ( hOptions.timestamp ? ( hOptions.align ? "\t" : " " ) : "" ) + "[" + ( hOptions.colorize === "all" || hOptions.colorize === "level" || hOptions.colorize === true ? winston.config.colorize( hOptions.level, sLevel ) : hOptions.level ) + "]";
	}
	if( hOptions.label ) sLog += ( hOptions.timestamp || hOptions.showLevel ? ( hOptions.align ? ( hOptions.showLevel && hOptions.level.length <= 4 ? "\t\t" : "\t" ) : " " ) : "" ) + hOptions.label;
	if( hOptions.timestamp || hOptions.showLevel || hOptions.label ) sLog += ":" + ( hOptions.align ? ( hOptions.label && hOptions.label.length <= 4 ? "\t\t" : "\t" ) : " " );
	sLog += ( hOptions.colorize === "all" || hOptions.colorize === "message" ? winston.config.colorize( hOptions.level, hOptions.message ) : hOptions.message );
...
let g_hLogFileOptions = { json: false, align: true, timestamp: GetTimestamp, formatter: FormatLog, maxsize: 5 * ( 1024 * 1024 ), maxFiles: 3, tailable: true, zippedArchive: true };
