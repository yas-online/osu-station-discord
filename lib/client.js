...
function JoinServers( hServers )
{
	for( let hServer of hServers )
	{
		...
		if( hVoiceChannel ) hVoiceChannel.join( false, false );
...
function OnMessage( hEvent )
{
	...
	if( !hMessage.isPrivate || GetOwner() != hAuthor ) return;
...
class Client /*extends EventEmitter*/
{
	constructor( hConfig )
	{
		...
		this.m_hClient.Dispatcher.on( "GATEWAY_READY", ( hEvent ) =>
		{
			...
			this.UpdateMetadata();
			this.m_hUpdateMetaTask = setInterval( () => ( this.UpdateMetadata() ), 15000 );
		} );
		...
		https.get( "https://radio.yas-online.net/listen/status-json.xsl", ( hResponse ) =>
		{
			...
			hResponse.on( "end", () =>
			{
				...
				if( hSource.title )
				{
					...
					if( hInfo ) this.m_hClient.User.setGame( hInfo[3] + " by " + hInfo[2] );
					else this.m_hClient.User.setGame( hInfo[1] + " by osu!station" );
				}
			} );
		} );
	}
	...
	StartStreaming( hConnection )
	{
		if( !this.m_hClient.VoiceConnections.length )
		{
			console.error( "Voice not connected" );
			return;
		}

		if( !hConnection ) hConnection = this.m_hClient.VoiceConnections[0].voiceConnection;
