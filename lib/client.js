...
function JoinServers( aServers )
{
	for( let hServer of aServers )
	{
		...
		else console.warn( "[" + hServer.name + "] Failed to find a osu!station voice channel" );
...
*/
				...
				this.m_hUpdateMetaTask.send( { game: this.m_hClient.User.gameName } );
			}
		} );
	...
	SetGame( Game = null )
	{
		if( this.IsConnected() && Game != this.m_hClient.User.gameName )
		{
			...
			if( this.m_hUpdateMetaTask && this.m_hUpdateMetaTask.connected ) this.m_hUpdateMetaTask.send( { game: Game } );
		}
	}
