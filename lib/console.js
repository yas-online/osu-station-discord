...
class Console
{
	constructor( hContext = null, bCreateConsole = true )
	{
		...
		if( bCreateConsole )
		{
			...
			{
				log.debug( "closing local console..." );

				this.Shutdown();
			} );
			if( hContext ) Object.assign( this.m_hConsole.context, hContext );

			log.debug( "creating remote socket..." );
			this.m_hSocket = net.createServer( ( hSocket ) =>
			{
				...
				if( hContext ) Object.assign( this.m_hRemoteConsole.context, hContext );
			} );
			this.m_hSocket.listen( g_sConsoleSocket, () =>
			{
				log.debug( "listening on remote socket" );
			} );
		}
		else
		{
