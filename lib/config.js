...
function UpdateConfig( pCallback )
{
	...
	{
		if( fs.existsSync( this.m_sFilePath ) )
		{
...
class Config
{
	...
	static SetPath( sPath )
	{
		if( typeof sPath !== "string" ) sPath = path.resolve( __dirname + "/../cfg" );
		if( !path.isAbsolute( sPath ) ) sPath = path.join( __dirname, sPath );
	...
	{
		if( g_sConfigPath === null ) Config.SetPath();

		try
		{
			fs.mkdirSync( g_sConfigPath );
		}
		catch( hException )
		{
		}
		...
		try
		{
			this.m_sFilePath = path.join( g_sConfigPath, path.dirname( sFile ), this.m_sFile );
		}
		catch( hException )
		{
			this.m_sFilePath = path.join( g_sConfigPath, this.m_sFile );
		}
...
Config.prototype[Symbol.toStringTag] = "Config";
